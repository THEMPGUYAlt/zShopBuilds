...
public class ItemBuilder extends ZUtils implements Cloneable {
    ...
    public ItemStack build() {
        ...
        if (flags != null) flags.forEach(flag -> meta.addItemFlags(flag));

        if (name != null) meta.setDisplayName(name);

        if (lore != null) meta.setLore(lore);

        if (enchantments != null) enchantments.forEach((e, l) -> meta.addEnchant(e, l, true));
    ...
    public ItemMeta getMeta() {
        if (meta == null) meta = new ItemStack(material, amount, (short) data).getItemMeta();
        return meta;
    }
