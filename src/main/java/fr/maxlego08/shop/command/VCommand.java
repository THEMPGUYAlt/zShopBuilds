...
import fr.maxlego08.shop.zcore.utils.commands.CollectionBiConsumer;
...
public abstract class VCommand extends Arguments {
     ...
     * @param plugin
     */
    public VCommand(ShopPlugin plugin) {
        super();
        this.plugin = plugin;
    }
     ...
     * @return
     */
    public CommandType prePerform(ShopPlugin plugin, CommandSender commandSender, String[] args) {
     ...
     */
    protected abstract CommandType perform(ShopPlugin plugin);
     ...
     * @return
     */
    public List<String> toTab(ShopPlugin plugin, CommandSender sender, String[] args) {
    ...
    public void syntaxMessage() {
        this.subVCommands.forEach(command -> {
            if (command.getPermission() == null || hasPermission(sender, command.getPermission())) {
                message(this.plugin, this.sender, Message.COMMAND_SYNTAX_HELP, "%syntax%", command.getSyntax(), "%description%", command.getDescription());
            }
        });
    }
